...
dataFinal <- read.csv("/Users/arihangupta/Downloads/PhiCB5-Analysis-2023/FISH_final")
...
p1
strainTest <- dataFinal[dataFinal$cond == 'ochre',]
tempDf <- strainTest[]
p1 <- ggplot(data = tempDf[tempDf$avgInt > 2822.568,], mapping = aes(x= avgInt-2822.568, fill = cond)) + geom_histogram(binwidth = 700) + facet_wrap(~time)
...
## Including Plots
```{r}
for (times in unique(dataFinal$time)){
  
  new<-tempDf[tempDf$time == times,]
  numCells <- nrow(new)
  
  flor <- new[new$avgInt > 2822.568,]
  numCellFluor <- nrow(flor)
  
  percent <- 100*(numCellFluor)/numCells
  print(paste("For ", times, ", total number of cells: ", numCells, " Number of fluorescent cells: ", numCellFluor, " Percent of cells the fluoresce: ", percent))
}
mean(dataFinal[dataFinal$id == '180_0nM-I_LZ22332',]$avgInt)
sd(dataFinal[dataFinal$id == '180_0nM-I_LZ22332',]$avgInt)
```
